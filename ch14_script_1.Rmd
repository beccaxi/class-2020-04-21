...
# parallel slopes model, since we use + 

# fitting model to try and predict Trump vote using all other variables

logistic_fit <- fit(logistic_mod, 
                    formula = trump ~ gender + age + educ + race + income + party + ideology, 
                    data = cces)
...
# 1) genderFemale offsets the intercept from the original Intercept (which is
# male) so the (largest) effect of being female is about a 8% less likely chance
# to vote for Trump (as opposed to being male)

# Also, we see that the coefficient on income is MUCH smaller than the
# coefficients on party and ideology, which understandably influence voting
# decisions far more than just income alone
...
# 2) this fits to 0, so Dom would not vote for Trump
```
...
randomForest <- fit(forest,
                    factor(trump) ~ gender + age + educ + race + income + party + ideology,
                    data = cces)

randomForest

# 1) Dom falls into not voting for Trump (the left-hand side)

# 2) OOB means error of 9.06%, or an accuracy of 90.94%.
